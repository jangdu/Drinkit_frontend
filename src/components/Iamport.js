...
export class RequestPay extends React.Component {
  requestPay = async (cartItems, input, user, usePoint, storeId) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_SERVERURL}/orders`,
        ...
        IMP.request_pay(
          ...
          async function (rsp) {
            ...
            if (rsp.success) {
              ...
              const response = await axios.post(
                `${process.env.REACT_APP_API_SERVERURL}/orders/postOrder`,
                {
                  totalPrice: rsp.paid_amount,
                  orderList,
                  impUid: rsp.imp_uid,
                  paidPoint: Number(usePoint),
                  storeId,
                  address: input,
                },
                {
  ...
  refund = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_SERVERURL}/orders/refund`,
        { imp_uid },
        {
          withCredentials: true,
          headers: {
